...
# Draw boxes on image
def draw_boxes(image, boxes, scores, labels, threshold=0.7):
...
def run_inference(image_path, model_path):
    ...
    # # Get class scores (excluding "no-object" class)
    # probs = logits.softmax(-1)[:, :-1]         # shape [num_queries, num_classes]

    # # Get max score and class index for each query
    # scores, labels = probs.max(-1)             # shape: [num_queries]

    # # Find the index of the highest scoring prediction
    # best_idx = scores.argmax()                 # scalar

    # # Select the best box, score, and label
    # boxes = boxes[best_idx].unsqueeze(0)       # shape [1, 4]
    # scores = scores[best_idx].unsqueeze(0)     # shape [1]
    # labels = labels[best_idx].unsqueeze(0)     # shape [1]
